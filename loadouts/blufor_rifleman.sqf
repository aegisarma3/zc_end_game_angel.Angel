scriptName "BLUFOR_Rifleman";

if (!hasInterface) exitWith { ["Loadout running on dedicated server - %1", "BLUFOR_Rifleman"] call BIS_fnc_logFormat; };
waitUntil { time > 0 };
waitUntil { !isNull player };
if (!local _this) exitWith { ["Loadout where unit is not local - %1 / %2", "BLUFOR_Rifleman", __this] call BIS_fnc_logFormat; };
